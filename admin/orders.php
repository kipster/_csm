...
*/
  ...
  $check_fidelity_query = tep_db_query("select o.fidelity, ot.value from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_TOTAL .  " ot where o.orders_id = '" . (int)$oID . "' and ot.orders_id = '" . (int)$oID . "' and ot.class = 'ot_total'");
  $check_fidelity = tep_db_fetch_array($check_fidelity_query);
  $points = $check_fidelity['fidelity'];
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'update_order':
        ...
        // originalif ( ($check_status['orders_status'] != $status) || tep_not_null($comments)) {
        ...
        $check_status2 = tep_db_fetch_array($check_status_query2);
          ...
          tep_db_query("insert into " . TABLE_ORDERS_STATUS_HISTORY . " (orders_id, orders_status_id, date_added, customer_notified, comments) values ('" . (int)$oID . "', '" . tep_db_input($status) . "', now(), '" . tep_db_input($customer_notified) . "', '" . tep_db_input($comments)  . "')");
			...
			//Create the fidelity points for this order if necessary:
			if ($points == 0){
				
                $order_attribute_query = tep_db_query("select products_options_values from " . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . " where orders_id = '" . (int)$oID . "'");
                $pp = 0;
                ...
                if ($check_fidelity['value'] > 52){$points = $check_fidelity['value'];} else {$points = $check_fidelity['value'] - $pp;}
				...
				$points = intval($points) * $multiplyby;

				$customer_points = tep_db_query("update " . TABLE_CUSTOMERS . " set points=points+" . $points . " where customers_id = '" . (int)$cID . "'");
				$order_points = tep_db_query("update " . TABLE_ORDERS . " set fidelity=" . $points . " where orders_id = '" . (int)$oID . "'");
			}
			//Create fidelity points eof--
...
// Fetch the fidelity points for this order:
if ($points == 0){
	$fidelity = 'Vos points sont en attente de paiement ou de validation, ils apparaitront bientot dans votre compte';
} else {
	$fidelity = 'Vous avez obtenu <b>' . $points . '</b> points avec cette commande';
}
...
?>
      ...
      <tr>
        <?php
        if (DOWNLOAD_ENABLED == 'true') include('includes/modules/download.php');
        ?>
      </tr>
      <tr>
        <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
